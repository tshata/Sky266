			...
			if( mainContainer.length > 0 ){
				$( mainContainer ).on( 'change keyup', 'input', function(){
					...
					$( mainContainer + ' tr' ).each(function(){
						...
						totalVolumetric += (currentVolumetric > 0) ? currentQTY * ( currentVolumetric / divisor ) : 1 *(( packageCbm * 1000000 )/divisor);
						totalWeight     += 1 * packageWeight;
						totalCbm        += (packageCbm > 0) ? 1 * packageCbm : 1 * ( currentVolumetric / 1000000 );
						totalVolume		+= 1 * currentVolumetric;
					});
