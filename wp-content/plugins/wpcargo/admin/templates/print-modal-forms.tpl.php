...
<div id="myModal" class="modal">
    ...
    <form id="modal_form" class="modal-content">
                ...
                <table id="wpcargo-package-table" class="wpc-multiple-package wpc-repeater">
                    <thead>
                        <tr>
                            ...
                            <th class="no-border-left no-border-right no-border-top no-border-bottom"
                                style="width: 2%;"></th>
                    ...
                    <tfoot>
                        ...
                        <tr class="wpc-computation">
                            <td colspan="6"><input data-repeater-create type="button" class="wpc-add"
                                    value="<?php esc_html_e('Add Package','wpcargo'); ?>" /></td>
                        </tr>
                    </tfoot>
                </table>
        ...
        <div class="modal-footer">
            ...
            <input type="hidden" id="package-weight" name="package-weight"
                value="<?php echo wpcargo_get_postmeta($shipment_id, 'package-weight', true); ?>">
            <input type="hidden" id="total_package-cbm" name="total_package-cbm"
                value="<?php echo wpcargo_get_postmeta($shipment_id, 'total_package-cbm', true); ?>">
            <input type="hidden" id="transport_mode" name="transport_mode"
                value="<?php echo wpcargo_get_postmeta($shipment_id, 'transport_mode', true); ?>">
            <input type="hidden" id="item_type" name="item_type"
                value="<?php echo (wpcargo_get_postmeta($shipment_id, 'item_type', true))? wpcargo_get_postmeta($shipment_id, 'item_type', true) : 'kg'; ?>">
            ...
            <a class="button" id="print_btn" style="float:right;" type="button" onclick="receipt_print()"><span
...
jQuery(document).ready(function($) {
    ...
    if ($("#tax_invoice").prop('checked') == true) {
        documents_switch(document.getElementById("tax_invoice"));
    } else if ($("#no_tax_invoice").prop('checked') == true) {
        $('#tax_invoice_yes_more').hide();
        if ($("#receipt").prop('checked') == true) {
            documents_switch(document.getElementById("receipt"));
        } else if ($("#no_receipt").prop('checked') == true) {
            documents_switch(document.getElementById("no_receipt"));
        }
    } else {
        $('#tax_invoice_yes_more').hide();
        $("#doc2").hide();
...
function re_calculate(elm) {
    ...
    var old_val = parseFloat(currentRow.find("td:eq(4)").text().replace('M', '').replace(':', '').replace(',', '')
        .replace('X', '').replace('-', ''));
    old_val = (old_val != "") ? old_val : 0;
    var new_val = (elm.value != "") ? parseFloat(currentRow.find("td:eq(2)").find("#price").val()) * parseFloat(
        currentRow.find("td:eq(3)").find("#qty").val()) : 0;
    currentRow.find("td:eq(2)").find("span").text((currentRow.find("td:eq(2)").find("#price").val() != "") ?
        formatMoney(parseFloat(currentRow.find("td:eq(2)").find("#price").val()), 2, ".", ",") : 0);
    currentRow.find("td:eq(3)").find("span").text((currentRow.find("td:eq(3)").find("#qty").val() != "") ? parseFloat(
        currentRow.find("td:eq(3)").find("#qty").val()) : 0);
    var total = parseFloat($("#total").text().replace(',', ''));
    var op_sign = (itemtype == "Expenditure") ? "-" : "";
    currentRow.find("td:eq(4)").html(op_sign + 'M ' + formatMoney(new_val, 2, ".", ",") +
        '<a style="color:red" href="#" onclick="remove_row(this)"> X</a>'); //update row total with new value
    var newtotal = (itemtype == "Expenditure") ? formatMoney(total - new_val + old_val, 2, ".", ",") : formatMoney(
        total + new_val - old_val, 2, ".", ",");
...
function add_row() {
    ...
    var value = ($("#item_select :selected").attr("data-value") != "") ? $("#item_select :selected").attr(
        "data-value") : 0;
    var op_sign = (itemtype == "Expenditure") ? "-" : "";
    var newtotal = (itemtype == "Expenditure") ? formatMoney(parseFloat($("#total").text().replace(',', '')) -
        parseFloat(value), 2, ".", ",") : formatMoney(parseFloat($("#total").text().replace(',', '')) + parseFloat(
        value), 2, ".", ",");
    $("#total").text(newtotal);
    var newrow = '<tr style="background:#E8E8E8;" data-type="' + itemtype + '"><td>' + label + '</td><td>' + unit +
        '</td><td>' + op_sign + 'M <input data-value="' + value + '" id="price" name="price" value="' + value +
        '" onkeyup="re_calculate(this)" ></td><td><input id="qty" name="qty" onkeyup="re_calculate(this)" data-value="1" value="1"></td><td>' +
        op_sign + 'M ' + value + '<a style="color:red" href="#" onclick="remove_row(this)"> X</a></td></tr>';
...
function remove_row(elm) {
    ...
    if (r == true) {
        ...
        var row_total = parseFloat($(elm).closest("tr").find("td:eq(4)").text().replace('M', '').replace(':', '')
            .replace(',', '').replace('X', '').replace('-', ''));
        var itemtype = $(elm).closest("tr").attr("data-type");
        var total = (itemtype == "Expenditure") ? formatMoney(parseFloat($("#total").text().replace(',', '')) +
            row_total, 2, ".", ",") : formatMoney(parseFloat($("#total").text().replace(',', '')) - row_total, 2,
            ".", ",");
        $("#total").text(total);
    }
}
...
function save_modal(elm) {
    ...
    var weight = (parseFloat($("#packages_div #package_volumetric").html()) > parseFloat($(
        "#packages_div #package_actual_weight").html())) ? parseFloat($("#packages_div #package_volumetric")
        .html()) : parseFloat($("#packages_div #package_actual_weight").html());
    ...
    if ($('#current_form').val() == "revise_quote") {
        ...
        $("#quote_div table tbody tr").each(function() {
            ...
            key_data['unit'] = self.find("td:eq(1)").text().replace('M', '').replace(':', '').replace(',', '')
                .replace('X', '').trim();
            key_data['price'] = self.find("td:eq(2) #price").val().replace('M', '').replace(':', '').replace(
                ',', '').replace('X', '').trim();
            key_data['qty'] = self.find("td:eq(3) #qty").val().replace('M', '').replace(':', '').replace(',',
                '').replace('X', '').trim();
            key_data['total'] = self.find("td:eq(4)").text().replace('M', '').replace(':', '').replace(',', '')
                .replace('-', '').replace('X', '').trim();
    ...
    $.ajax({
        ...
        success: function(data) {
            if ($('#current_form').val() == "revise_quote") {
                ...
                if ($('#current_form').val() == "revise_quote") {
                    $('#invoice_btn').text("Save Invoice");
                    $('#invoice_btn').show();
                    $('#invoice_btn').text("Create Invoice");
                }
            } else {
...
function quote_more(clicked_btn, post_id, option) {
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#back_btn').text("Close");
            if (option == "wpcargo_invoice" && clicked_btn.id == "new_invoice") {
                $('#invoice_btn').show();
                $('#invoice_btn').text("Create Invoice");
                $('#print_btn').hide();
...
function save_invoice() {
    ...
    if (r == true) {
        ...
        $("#quote_div table tbody tr").each(function() {
            ...
            key_data['unit'] = self.find("td:eq(1)").text().replace('M', '').replace(':', '').replace(',', '')
                .replace('X', '').trim();
            key_data['price'] = self.find("td:eq(2) span").text().replace('M', '').replace(':', '').replace(',',
                '').replace('X', '').trim();
            key_data['qty'] = self.find("td:eq(3) span").text().replace('M', '').replace(':', '').replace(',',
                '').replace('X', '').trim();
            key_data['total'] = self.find("td:eq(4)").text().replace('M', '').replace(':', '').replace(',', '')
                .replace('-', '').replace('X', '').trim();
            table_data[key] = key_data;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                amount_paid = (parseFloat($("#print-shipper-info #amount_paid_inf").text().replace(',',
                    ''))).toFixed(2);
                $("#print-shipper-info #amount_due").text(formatMoney(parseFloat($("#total").text().replace(
                    ',', '')) - amount_paid, 2, ".", ","));
...
function save_payment() {
    ...
    if ($('#payment_date').val() == "") {
        $('#payment_form #msg_box').text("Date cannot be empty");
        return false;
    } else if ($('#payment_method').val() == "") {
        $('#payment_form #msg_box').text("Select Payment Method");
        return false;
    } else if ($('#payment_reference').val() == "" && payment_method != "Account") {
        $('#payment_form #msg_box').text("Payment reference cannot be empty");
        return false;
    } else if ($('#payment_amount').val() <= 0 && payment_method != "Account") {
        $('#payment_form #msg_box').text("Payment Amount cannot be less than or equal to 0");
        return false;
    }
    ...
    if (confirm_pay) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                amount_paid = (parseFloat($("#print-shipper-info #amount_paid_inf").text().replace(',',
                    '')) + parseFloat($('#payment_form #payment_amount').val().replace('M', '')
                    .replace(
                        ',', ''))).toFixed(2);
                ...
                $("#print-shipper-info #amount_paid_inf").text(formatMoney(parseFloat(amount_paid), 2, ".",
                    ","));
                $("#print-shipper-info #amount_due").text(formatMoney((parseFloat($(
                        "#print-shipper-info #invoice_inf").text().replace(',', '')) -
                    amount_paid), 2, ".", ","));
                $('#quote_div').html(data);
                $('#print_btn').show();
...
function status_form(elm) {
    ...
    if (elm.value == "Payment Approval") {
        $('.wpcargo-loading').show();
        payment_more($('#post_id').val());
    } else if (elm.value == "Invoicing") {
        $('.wpcargo-loading').show();
        quote_more(elm, $('#post_id').val(), "wpcargo_price_estimates");
    } else if (elm.value == "Collection") {
        $("#state_div #state_div_extra").html("<label>" + elm.value +
            " Status:  &nbsp;</label><select name='remarks' value=''><option value='" + elm.value +
            " Successful'>Successful</option><option value='" + elm.value + " Failed'>Failed</option></select>");
    } else if (elm.value == "Delivery") {
        $("#state_div #state_div_extra").html("<label>" + elm.value +
            " Status:  &nbsp;</label><select name='remarks' value=''><option value='" + elm.value +
            " Successful'>Successful</option><option value='" + elm.value + " Failed'>Failed</option></select>");
    }
...
function receipt_print() {
    ...
    style = style +
        "#receipt_display table tbody .row td, table thead .row td{ border-right: 2px solid; border-bottom: 2px solid; }";
    style = style +
        "#receipt_display table tbody .row td:nth-child(5) , table thead .row td:nth-child(5){ border-right: none; }";
    style = style + "</style>";
