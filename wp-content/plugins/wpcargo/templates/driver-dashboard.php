    ...
    <div id="content_div">
       ...
       <?php
            ...
            <table class="datatable row-border" id="trips_table_list">
              <thead>
               <tr><th>#</th><th>Trip Name</th><th>Trip Date</th><th>Assigned Drivers</th><th>Status</th></tr>
              </thead>
              <tbody>
              <?php  $i=1;
              foreach($comingtrips AS $trip){      ?>
                  <tr id="<?php echo $trip->id; ?>" class='clickable-row'>
                      ...
                      <td style='background:<?php echo ($trip->status=="Active")?"green" : (($trip->status=="Upcoming")?"orange" : (($trip->status=="Terminated")?"red" : "gray")); ?>'>
  ...
  <form id="myModal" class="wpcargo-modal" >
              ...
              <div style=" display: none;" id="trip_div">
                  <div style=" display: none;" id="activate"  class=" wpcargo-row">
                    ...
                    <div class="wpcargo-col-md-9">
                        <input type="text" name="vehicle_reg"  placeholder="0" style="width: 200px; height: 25px; background: none;" />
                    </div>
                    <div class="wpcargo-col-md-3"> Vehicle Mileage :</div>
                    <div class="wpcargo-col-md-9">
                        <input type="text" name="opening_mileage"  placeholder="0" style="width: 200px; height: 25px; background: none;" />
                    </div>
                    <div class="wpcargo-col-md-3">Trailer Reg. : </div>
                    <div class="wpcargo-col-md-9">
                        <input type="text" name="trailer_reg"  placeholder="0" style="width: 200px; height: 25px; background: none;" />
                    </div>
                    <div class="wpcargo-col-md-3">Trip Checklist : </div>
                        <ul id="checklist" class="wpcargo-col-md-9">
                             <li><input type="checkbox" class="abc" style="width: 20px; height: 15px;" />&nbsp;&nbsp;License</li>
                             <li><input type="checkbox" class="abc" style="width: 20px; height: 15px;" />&nbsp;&nbsp;Passport</li>
                        </ul>
                  </div>
                  <div style=" display: none;" id="end">
                       Car Mileage: <input type="text" name="closing_mileage"  placeholder="0" style="width: 200px; background: none;" />
                  </div>
              </div>
              <div style=" display: none;" id="shipment_div">
                  <div id="success">
                       ...
                       <table id="wpcargo-package-table" class="wpc-multiple-package wpc-repeater">
                          <thead>
                            <tr><th>Barcode</th><th>AW</th><th>DW</th><th>CBM</th><th>&nbsp;</th></tr>
                          </thead>
                          <tbody data-repeater-list="<?php echo WPCARGO_PACKAGE_POSTMETA; ?>">
                            <tr data-repeater-item class="wpc-mp-tr">
                                <?php $i=1;
                                 ?>
                                <td><input type="text" name="wpc-pm-scanner[]" id="wpc-pm-scanner" placeholder="Barcoder"></td>
                                <td><input type="text" name="wpc-pm-weight[]" id="wpc-pm-weight" placeholder="AW"></td>
                                <td><input type="text" name="wpc-pm-length[]" id="wpc-pm-length" placeholder="L" style="width: 30%;">
                                    <input type="text" name="wpc-pm-width[]" id="wpc-pm-width" placeholder="W" style="width: 30%;">
                                    <input type="text" name="wpc-pm-height[]" id="wpc-pm-height" placeholder="H" style="width: 30%;">
                                </td>
                		  ...
                		  <tfoot>
                			<tr class="wpc-computation">
                				<td colspan="5">
                				    <input data-repeater-create type="button" class="wpc-add" value="<?php esc_html_e('Add Package','wpcargo'); ?>"/>
                                 </td>
                			</tr>
                		  </tfoot>
                       </table>
    ...
    function driver_city_singleview(btn){
      ...
      function update_trip_state(elm){
          ...
          else {  var unattended = 0;
             $("#bookings_table_list tbody tr").each(function () {
                    unattended += parseInt($(this).find("td:eq(2)").text().trim());
              });
             if(unattended > 0) {alert("Trip cannot be ended! Because it has "+unattended+" unattended bookings."); return false;}
      ...
      function update_shipment_state(elm){ $('#save_btn').show();
          ...
          $('body').append('<div class="wpcargo-loading">Loading...</div>');
             $.ajax({
                  ...
                  success:function(data) {  
                      ...
                      else if($("#current_form").val()=="success_form" || $("#current_form").val()=="failed_form"){
