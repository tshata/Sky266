<div id="wpc-multiple-package" class="print-section wpcargo-table-responsive table-responsive">
    <p class="header-title">
        <strong><?php apply_filters( 'wpc_multiple_package_header', esc_html_e( 'Packages', 'wpcargo' ) ); ?></strong>
        <?php if( is_admin() ){ ?> &nbsp;&nbsp;<a class="button button-primary" type="button"
            onclick="wpcargo_revise_packages()"><span class="dashicons dashicons-clipboard"></span>
            <?php echo apply_filters( 'wpcargo_revise_packages_label', esc_html__( 'Revise Packages', 'wpcargo') ); ?></a>
        <?php } ?>
    ...
    <table class="table wpcargo-table" id="packages_final" style="width:100%;">
        ...
        <tbody>
            ...
            <tr>
                <td class="empty-data"
                    colspan="<?php echo !wpcargo_package_settings()->dim_unit_enable ? count( wpcargo_package_fields() ) - count( wpcargo_package_dim_meta() ) : count( wpcargo_package_fields() ) ; ?>">
                    <i>Data empty.</i>
                </td>
            </tr>
            <?php endif; ?>
