...
?>
 ...
 <div id="payment_items" class="shipping-form">
     <!-- form step tree -->
     ...
     <div class="wpcargo-row" style="padding:10px;">
         ...
         <div class="wpcargo-col-md-6" style="padding-left: 4px;">
             ...
             ?>
                 ...
                 <p>To Activate this Booking, Please pay a minimum amount of <span><strong>M
                             <?php echo number_format((float)$bookingfee, 2, '.', ','); ?></strong></span></p><br>
                 <input type="hidden" id="bookingFee"
                     value="<?php echo number_format((float)$bookingfee, 2, '.', ','); ?>" />
                 <input style="width: 15px; height: 15px;" type='checkbox' name="mobileMpesa" id="mobileMpesa"
                     onclick="payments_toggle(this)" data-group='payments'><span>Mpesa</span><br>
                 <input style="width: 15px; height: 15px;" type='checkbox' name="mobileEco" id="mobileEco"
                     onclick="payments_toggle(this)" data-group='payments'><span>Ecocash </span><br>
                 <input style="width: 15px; height: 15px;" type='checkbox' name="bank" id="bank"
                     onclick="payments_toggle(this)" data-group='payments'><span>Bank EFT/Deposit</span><br>
                 <input style="width: 15px; height: 15px;" type='checkbox' name="cash" id="other"
                     onclick="payments_toggle(this)" data-group='payments'><span>Other</span><br><br><br>
         ...
         <br>
         <p id="info" style="<?php echo (empty($wpcargo_payment_history))? 'display:none;': ''; ?>">
             <?php echo "You have paid M".number_format((float)$amount_paid, 2, '.', '')." of the Total amount of M".number_format((float)$total_price, 2, '.', '')." Remaining Balance is M".number_format((float)($total_price-$amount_paid), 2, '.', '');?>.
         </p>
...
function payments_toggle(btn) {
    ...
    if (btn.id == "mobileMpesa") {
        ...
        document.getElementById("otherMethods").style.display = "none";
        document.getElementById("reference_div").style.margin = "2.8em 0 0 0";
        var bankNames = "PAY TO:  STANDARD LESOTHO BANK: 9080007532411" + "<br/>" + "FIRST NATIONAL BANK: " + "<br/>" +
            "\tNEDBANK:";
